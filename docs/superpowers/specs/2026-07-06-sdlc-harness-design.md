...
skills/
  sdlc/                     # the one installed skill; invocable as /sdlc
    ...
    workflows/              # invoked via the Workflow tool's scriptPath
      review.js             # Phase 0 uses inline explorers (no workflow)
...
- **Workflow scripts** (`workflows/review.js`) are bundled resource files invoked via the Workflow tool's `scriptPath`, used *surgically* for the one remaining fan-out-heavy phase (Review), which uses a find → adversarially-verify pattern so plausible-but-wrong findings don't survive.
