...
skills/
  sdlc/                     # the one installed skill; invocable as /sdlc
    ...
    scripts/                # deterministic Node helpers (unit-tested)
      lib/  scaffold.mjs  new-task.mjs  status.mjs  write-memory.mjs
      set-state.mjs  loop.mjs  review.mjs
...
1. **Investigate** — the `understand` phase guide has the conductor dispatch parallel `explorer` subagents *inline* via the Agent tool (prompt from `agents/explorer.md`), each mapping a slice: structure & entry points, tech stack & dependencies, key runtime flows, build/test/run mechanics, conventions, and fragile/risky areas. No Workflow-tool dependency — portable.
2. **Extract** — the conductor merges the explorers' structured findings into one findings object (the defined schema).
3. **Persist** — a deterministic `scripts/write-memory.mjs` renders the findings into `.sdlc/memory/*.md`. If a code-graph MCP is available and `memory.graph` is `auto`/`on`, also build a graph index for structural queries; otherwise fall back to markdown only.
...
- **Phase 0 investigation** fans out `explorer` subagents *inline* (Agent tool) and merges their findings; a deterministic `scripts/write-memory.mjs` renders the memory files. No Workflow-tool dependency.
- **Review** fans out `reviewer` subagents *inline* (one per dimension, Agent tool), then `verifier` subagents that adversarially try to refute each finding (majority rule); a deterministic `scripts/review.mjs` dedupes findings, tallies verdicts, and renders `review.md`. Only verified-`real` findings loop back to Implement, so plausible-but-wrong findings don't churn the loop.
- **No Workflow-tool dependency anywhere.** The harness uses inline Agent fan-out for every phase and ships zero Workflow-tool scripts, keeping it portable across environments.
...
2. Phase 0: inline explorer fan-out + findings merge + `write-memory.mjs` + `understand` phase guide, wired into `/sdlc init`.
