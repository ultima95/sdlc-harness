...
2. **Finalize, push, and open the PR.** Make sure all work is committed on the feature branch —
   including the task's `.jig/` state (`spec.md`/`progress.md`/`review.md`/`state.json`) when
   `git.track_state`. Read `git.*` and `ship.mode` from `.jig/config.yml`. `trust_level` governs confirmations:
...
4. **Refresh Project Memory** (invariant — memory must never go stale). Per
   `.jig/config.yml` `memory.refresh` (default `on_ship`):
   - `on_ship`: apply targeted, index-first updates to the memory files the change touched
     ...
     structural change, run a full re-index instead: `/jig memory-refresh`.
   - `manual`: skip here; the developer runs `/jig memory-refresh` when they choose.
5. Record the outcome:
   `node "<SKILL_DIR>/scripts/progress.mjs" "<taskDir>" ship "<what shipped: branch/PR + memory refreshed>"`
   Then, when `git.track_state`, commit the refreshed `.jig/` (memory + progress + state) on the
   feature branch — a standalone `.jig/` commit — and `git push` so it's part of the PR (see
   SKILL.md § Committing `.jig/` state).
6. Close or hand off — depends on whether there is a branch to clean up later:
   - If a feature branch exists (a `branch` is recorded in `state.json`) AND `git.cleanup:
     ...
     Tell the developer to run `/jig cleanup <taskId>` once the PR is merged.
